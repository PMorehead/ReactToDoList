...
class ToDoList extends React.Component {
    ...
    fetchTasks() {
        fetch("https://fewd-todolist-api.onrender.com/tasks?api_key=1157").then(checkStatus).then(json).then((response) => {
    ...
    handleSubmit(event) {
        ...
        fetch("https://fewd-todolist-api.onrender.com/tasks?api_key=1157", {
            method: "POST",
            mode: "cors",
    ...
    deleteTask(id) {
        ...
        fetch(`https://fewd-todolist-api.onrender.com/tasks/${id}?api_key=1157`, {
            method: "DELETE",
            mode: "cors",
        }).then(checkStatus).then(json).then((data) => {
            this.fetchTasks();
        }).catch((error) => {
            this.setStae({ error: error.message });
            console.log(error);
        })
    }
    ...
    toggleComplete(id, completed) {
        ...
        fetch(`https://fewd-todolist-api.onrender.com/tasks/${id}/mark_${newState}?api_key=1157`, {
            method: "PUT",
            mode: "cors",
        }).then(checkStatus).then(json).then((data) => {
            this.fetchTasks();
                ...
                <div className="row">
                    <div className="col-12">
                        ...
                        {tasks.length > 0 ? tasks.map((task) => {
                            ...
                            return (
                                <Task key={task.id} task={task} onDelete={this.deleteTask} onComplete={this.toggleComplete} />
                            );
                        }) : <p>no tasks here</p>}
                        <div className="mt-3">
                            <label>
